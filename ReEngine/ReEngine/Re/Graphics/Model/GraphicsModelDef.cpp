...
namespace Graphics
{
	...
	void ModelDef::setSprite(sf::Sprite & toUpdate, const sf::Transformable& parent) const
	{
		...
		toUpdate.setColor(sf::Color(
			(sf::Uint8)clamp<float>(color.r, 0.f, 255u),
			(sf::Uint8)clamp<float>(color.g, 0.f, 255u),
			(sf::Uint8)clamp<float>(color.b, 0.f, 255u),
			(sf::Uint8)clamp<float>(color.a, 0.f, 255u)
		));
	...
	void ModelDef::setSprite(sf::Sprite & toUpdate) const
	{
		...
		toUpdate.setColor(sf::Color(
			(sf::Uint8)clamp(color.r, 0.f, 255.f),
			(sf::Uint8)clamp(color.g, 0.f, 255.f),
			(sf::Uint8)clamp(color.b, 0.f, 255.f),
			(sf::Uint8)clamp(color.a, 0.f, 255.f)
		));
		toUpdate.setOrigin(toUpdate.getTextureRect().width*0.5f + origin.x,
			toUpdate.getTextureRect().height*0.5f + origin.y);
	}
